...
public:
	...
	void setup(){
		...
		// Add fine adjust factor as an inspector parameter
		...
		addCustomRegion(customWidget, [this](){
			...
			if (values_count >= 1)
			{
				...
				auto mousePosPrev = mousePos - mouseDelta;
				
				// Use modified mouse position for fine adjustment
				ImVec2 adjustedMousePos = mousePos;
				bool usingFineAdjust = ImGui::GetIO().KeyAlt;
				
				if (usingFineAdjust && ImGui::IsItemActive() && ImGui::IsMouseDragging(0, 0)) {
					// Only adjust the Y position for fine control
					// Scale the Y delta by the fine adjustment factor
					float scaledDeltaY = mouseDelta.y * fineAdjustFactor;
					
					// Apply scaled delta to previous position instead of using current mouse position
					adjustedMousePos.y = lastAdjustedMousePos.y + scaledDeltaY;
					
					// Store for next frame
					if (ImGui::IsMouseClicked(0)) {
						// Reset on first click
						lastAdjustedMousePos = mousePos;
					} else {
						lastAdjustedMousePos = adjustedMousePos;
					}
				} else {
					// Normal behavior - store actual mouse position
					lastAdjustedMousePos = mousePos;
				}
				...
				if (ImGui::IsItemActive() && ImGui::IsMouseDragging(0, 0))
				{
					...
					// Use adjusted mouse position for Y when fine tuning
					float nVal0;
					if (usingFineAdjust) {
						nVal0 = 1 - ImClamp((adjustedMousePos.y - inner_bb.Min.y) / (inner_bb.Max.y - inner_bb.Min.y), 0.0f, 1.0f);
					} else {
						nVal0 = 1 - ImClamp((mousePos.y - inner_bb.Min.y) / (inner_bb.Max.y - inner_bb.Min.y), 0.0f, 1.0f);
					}
					
					// Always use actual previous mouse position for nVal1
...
private:
	...
	// For fine adjustment tracking
	ImVec2 lastAdjustedMousePos;
};

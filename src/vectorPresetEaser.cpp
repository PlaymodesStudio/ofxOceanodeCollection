...
vectorPresetEaser::vectorPresetEaser() : ofxOceanodeNodeModel("Vector Preset Easer"){
    addParameter(time.set("time", 0.5, 0, 5));
    addParameter(input.set("Input", {0}, {0}, {1}));//.isSavePreset = false;
...
void vectorPresetEaser::update(ofEventArgs &a){
    if(isTweening){
        ...
        if(lastInputStore.size() == input.get().size()){
            float val = ofMap(timeElapsed, 0, time, 0, 1);//ofxeasing::map(timeElapsed, 0, time, 0, 1, ofxeasing::linear::easeNone);
